...
flux2$leg <- gsub("leg_", "", flux2$column_label)


ggplot()+
  geom_point(data=dm, aes(x=date, y=HGT_RADR), size=1, colour="grey") +
  geom_point(data=dm, aes( x=date, y=BLH), size=1, colour="black") +
  geom_point(data=flux2, aes( x=date, y=alt, colour=leg), size=2) +
  theme_bw()+
...
ggplot(flux2)+
  ...
  theme(plot.title = element_text(hjust = 0.5), 
        text = element_text(size=14)) +
  labs(x="Altitude (m)", y=bquote(''~EC~(CH[4]~mg~m^-2~h^-1)*''), title="C137", colour="% correction")
